...
public class Estudiante extends Persona {
  ...
  try{
   ...
   cn.abrir_conexion();
    String query;
    query = "Select idEstudiantes as id,nit,nombres,apellidos,direccion,telefono,fecha_nacimiento from clientes;";
  ...
  return tabla;
   ...
   public void actualizar(){
         ...
         cn.abrir_conexion();
         String query;
        query = "update estudiantes set carnet = ?,nombres= ?,apellidos= ?,direccion= ?,telefono= ?,genero= ?,email= ?fecha_nacimiento= ? "+ "where idEstudiante = ?";
         parametro  = (PreparedStatement) cn.conexionBD.prepareStatement(query);
         parametro.setString(1, getCarnet());
         parametro.setString(2, getNombres());
         parametro.setString(3, getApellidos());
